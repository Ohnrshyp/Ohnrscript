...
echo "────────────────────────────────────────────────────────"
echo "  tls.ohn — Native TLS Server Build (mbedTLS)"
...
echo "▶ Linking server.ll + bindings.c → tls.ohn binary..."
 if ! $CLANG -O3 $MBED_CFLAGS -o "$OUT_DIR/tls.ohn" "$OUT_DIR/server.ll" "$PKG_DIR/src/bindings.c" "$REPO_ROOT/compiler/src/shim/ohnrscript-runtime.c" -lm $MBED_LDFLAGS; then
    echo "ERROR: Linking failed."
    exit 1
fi
echo ""
echo "────────────────────────────────────────────────────────"
echo "  ✓ Build Complete: dist/tls.ohn"
echo ""
echo "  Run:"
echo "    cd packages-llvm/tls.ohn && ./dist/tls.ohn"
echo ""
echo "  Test:"
echo "    curl -k -v https://localhost:8443"
echo "────────────────────────────────────────────────────────"

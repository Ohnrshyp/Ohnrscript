...
/* ============================================================
 * 9. TLS (mbedTLS) ZERO-ALLOCATION BRIDGE
 * ============================================================ */
#include <sys/types.h>
...
void sys_tls_init(void) {
    ...
    mbedtls_x509_crt_parse_file(&default_cert_node.cert, "cert.pem");
    mbedtls_pk_parse_keyfile(&default_cert_node.pkey, "key.pem", NULL, NULL, NULL);
    ...
    mbedtls_ssl_conf_own_cert(&conf, &default_cert_node.cert, &default_cert_node.pkey);
...
int64_t sys_tls_connect(int32_t slot, int32_t fd, int64_t hostname_ptr) {
    ...
    if (hostname_ptr != 0) {
        char* hostname = (char*)(uintptr_t)hostname_ptr;
...
int64_t sys_tls_inject_ca(int32_t slot, int64_t ca_str_ptr, int32_t ca_len) {
    if (slot < 0 || slot >= 10240) return -1;
    mbedtls_ssl_context* ssl = &ssl_contexts[slot];
    ...
    unsigned char* ca_str = (unsigned char*)(uintptr_t)ca_str_ptr;
